function FormTestLL_Constructor(pGUI, luacontext)
	...
	local radiogroup = pGUI:GetViewById("formTestRadioGroup");
	radiogroup:RegisterEventFunction("CheckedChanged", LuaTranslator.Register(radiogroup, "TestRadioGroup_CheckedChanged"));
end

function TestRadioGroup_CheckedChanged(pGUI, context, val)
    LuaToast.Show(context, "RadioGroup value is " .. val, 1000);
end
